...
class PlayState extends MusicBeatState
{
	...
	var bloom:BloomShader = new BloomShader();
	//var blur:BlurShader = new BlurShader();
	...
	override public function create()
	{
		...
		if(!ClientPrefs.lowQuality && ClientPrefs.shaders){
			...
			shader.barrelDistortion1 = -0.25;
			shader.barrelDistortion2 = -0.25;
			camGame.setFilters([new ShaderFilter(bloom), new ShaderFilter(shader)]);
		}
		...
		healthBarBG.scale.set(0.7, 0.7);
		healthBarBG.updateHitbox();
		healthBarBG.y = FlxG.height;
		...
		if(ClientPrefs.downScroll){ 
			...
			healthBarBG.y -= healthBarBG.height;
			healthBarBG.y += 20;
		}

		healthBar = new FlxBar(healthBarBG.x - 50, healthBarBG.y + ((ClientPrefs.downScroll ? 72 : 85)), RIGHT_TO_LEFT, 1051 - 8, 23 - 8, this,
			'health', 0, 2);
		...
		//if(ClientPrefs.downScroll)
		//	healthBar.offset.y += 50;
		add(healthBar);
		add(healthBarBG);
		
		iconP1 = new HealthIcon(boyfriend.healthIcon, true);
		iconP1.scale.set(0.7, 0.7);
		iconP1.y = healthBarBG.y;
		iconP1.x = healthBarBG.width - 300;
		...
		var textP1 = new FlxSprite().loadGraphic(Paths.image("iconNames/bf"));
		textP1.scale.set(0.7, 0.7);
		textP1.x = iconP1.x - 120;
		textP1.y = iconP1.y;
		add(textP1);
		var textP2 = new FlxSprite().loadGraphic(Paths.image("iconNames/jellie"));
		textP2.scale.set(0.7, 0.7);
		textP2.x = iconP2.x;
		textP2.y = iconP2.y;
		add(textP2);
		...
		scoreTxt = new FlxText(0, healthBarBG.y + 110, FlxG.width, "", 20);
		...
		textP1.cameras = [camHUD];
		textP2.cameras = [camHUD];
	...
	function startSong():Void
	{
		...
		wave = chromaVideo("jellyride");
		wave.alpha = 0;
		FlxTween.tween(wave, {alpha: 1}, 0.5, {ease: FlxEase.circOut});
		wave.y = FlxG.height - 136 - healthBarBG.height/2;
		wave.y -= 10;
		wave.x = (FlxG.width - 598) / 2;
		if(ClientPrefs.downScroll){
			wave.flipY = true;
			wave.y = 136 - 25;
			wave.y -= 20;
	...
	override function openSubState(SubState:FlxSubState)
	{
		if (paused)
		{
			...
			if (wave != null)wave.bitmap.pause();
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if(wave!=null){
				wave.bitmap.resume();
				wave.bitmap.time = Std.int(Conductor.songPosition);
			}
	...
	override public function update(elapsed:Float)
	{
		...
		bloom.amount = FlxMath.lerp(bloom.amount, 0.5, (elapsed / (1/60)) * 0.035);
	...
	override function beatHit()
	{
		...
		if (iconP2.animation.curAnim.name == 'bop0') {
			iconP2.animation.play('bop1', true);
		} else {
			iconP2.animation.play('bop0', true);
		}
		if (iconP1.animation.curAnim.name == 'bop0') {
			iconP1.animation.play('bop1', true);
		} else {
			iconP1.animation.play('bop0', true);
		}
